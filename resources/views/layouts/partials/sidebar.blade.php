...
<aside class="sidebar sidebar-light #sidebar-expand-md">
    ...
    @php
        ...
        $super_access=session()->has('user') && session()->get('user')->is_superadmin==1 ? true : false;
        $user_access=!$super_access && count(session('relatedOrganizations')) > 0 ? true : false;
    @endphp

    @if($super_access || $user_access)
            ...
            @if(!empty(session()->get('user')))
                ...
                <span class="dropdown-toggle no-caret" data-toggle="dropdown">
                    <div class="profile-info">
                        <img class="avatar" src="{{session()->has('organization') ? session()->get('organization')->featured_image : env('UI_AVATAR').'NA'}}" alt="...">
                        <h4 class="mb-0">
                            @php $defaultOrganization = session()->get('organization')  @endphp
                            {{ isset($defaultOrganization->name) ? $defaultOrganization->name : null }}
                            <i class="fa fa-caret-down"></i>
                        </h4>
                        <p>{{ isset(session('user')->roles) ? implode(',',array_column(session()->get('user')->roles,'name')) : '' }}</p>
                    </div>
                </span>
