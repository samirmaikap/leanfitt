...
@section('content')
    ...
    <script>
        ...
        window.onload = function () {
            ...
            drake.on('drop', function (el, source, handle, sibling) {
                var currentIndex = $(el).index();
                $(el).find('form input[name="position"]').val(currentIndex);
                console.log('current index: ', currentIndex, $(sibling).index())
            });
        ...
        function initDropzone(selector, options) {
            ...
            return new Dropzone("#dropzone", options);
